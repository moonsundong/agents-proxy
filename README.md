...
## 🆚 为什么不直接用 LiteLLM / One-API?

这些项目都很优秀,但它们解决的问题和本网关不同:

| 项目 | 它的"路由"是什么 | 按请求复杂度智能分流? | 上下文压缩? | Web 管理 UI? |
| --- | --- | :-: | :-: | :-: |
| One-API / New-API | 渠道管理 + Key 分发 + 按模型名转发 | ❌ | ❌ | ✅ |
| LiteLLM | **负载均衡型**:轮询 / 延迟最低 / 失败转移,解决"哪个实例活着" | ❌ | ❌ | 部分 |
| Portkey | **规则型**:按预先写的 metadata 条件分发 | ❌ | ❌ | ✅ |
| RouteLLM | **复杂度路由**(思路最接近),但需自行训练/配置路由器,偏研究框架 | ✅ | ❌ | ❌ |
| **本网关** | **决策模型实时评估每道题的难度**,简单走本地、难走线上 | ✅ | ✅(Headroom) | ✅ |

一句话:**它们的路由解决"可用性"和"规则分发",我们的路由解决"这道题值不值得花钱"**。
再加上 Headroom 压缩,这是目前唯一同时砍 **上下文成本** 和 **模型差价** 两笔钱的方案 —— 两道省钱可以叠加。
